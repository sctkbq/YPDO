...
def damage_gauge(character_table, battle_data):
    ...
    for i in character_table:
        if i.startswith("char_"):
            ...
            inst_id_char_name[inst_id] = character_table[i]["name"]

    print("----------")
    print("Damage Gauge:")

    for i in battle_data["battleData"]["stats"]["charList"]:
        inst_id = int(i)
        char_data = battle_data["battleData"]["stats"]["charList"][i]

        try:
            # dmg calculation could be changed by future versions, unstable
            damage = -char_data[4][0]

            if damage:
                print(f"{inst_id_char_name[inst_id]}: {damage}")
        except Exception:
            print(f"Failed to get {inst_id_char_name[inst_id]}'s damage.")
    ...
    print(
        json.dumps(
            ...
            ["stats"]["extraBattleInfo"], indent=4
        )
    )
