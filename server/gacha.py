...
def doGetPool(poolId):
    ...
    if is_valid:
        ...
        if os.path.isfile(pool_file):
            ...
            gachaTemp = read_json(GACHA_TEMP_JSON_PATH)
            if "fesClassic" in gachaTemp and poolId in gachaTemp["fesClassic"]:
                pool["detailInfo"]["upCharInfo"] = {
                    "perCharList": [
                        {
                            "rarityRank": int(i),
                            "charIdList": gachaTemp["fesClassic"][poolId][i],
                            "percent": 0.5/len(gachaTemp["fesClassic"][poolId][i]),
                            "count": len(gachaTemp["fesClassic"][poolId][i])
                        } for i in gachaTemp["fesClassic"][poolId]
                    ]
                }
            return pool
...
def choosePoolUp():
    request_json = request.json
    poolId = request_json["poolId"]
    upChar = request_json["chooseChar"]

    gachaTemp = read_json(GACHA_TEMP_JSON_PATH)
    if "fesClassic" not in gachaTemp:
        gachaTemp["fesClassic"] = {}
    gachaTemp["fesClassic"][poolId] = upChar
    write_json(gachaTemp, GACHA_TEMP_JSON_PATH)
