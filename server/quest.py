...
from utils import read_json, write_json, decrypt_battle_data, damage_gauge
...
def questBattleFinish():
    ...
    if config["userConfig"]["damageGauge"]:
        character_table = updateData(CHARACTER_TABLE_URL)
        damage_gauge(
            character_table,
            decrypt_battle_data(request.json["data"])
        )
        ...
        "assistList": [
            {
                ...
                "assistCharList": [
                    assist_unit, None, None
                ],
                "powerScore": 500,
                "isFriend": True,
                "canRequestFriend": False,
                "assistSlotIndex": 0
            } for i, assist_unit in enumerate(assist_units)
        ],
        "playerDataDelta": {
            "modified": {},
            "deleted": {}
