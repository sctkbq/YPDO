...
def accountSyncData():
    ...
    for i in character_table:
        if "char" not in operatorKeys[cnt] or operatorKeys[cnt] == "char_512_aprot":
            cnt += 1
            continue
        ...
        if myCharList[int(cntInstId)]["evolvePhase"] == 2:
            phase_2_skin = str(operatorKeys[cnt]) + "#2"
            if phase_2_skin in data_skin["charSkins"]:
                myCharList[int(cntInstId)]["skin"] = phase_2_skin
        ...
        if operatorKeys[cnt] == "char_002_amiya":
            ...
            for tmpl_id in ["char_002_amiya", "char_1001_amiya2", "char_1037_amiya3"]:
                if tmpl_id in equip_table["charEquip"]:
                    for equip in equip_table["charEquip"][tmpl_id]:
                        level = 1
                        if equip in list(battle_equip_table.keys()):
                            level = len(battle_equip_table[equip]["phases"])
                        myCharList[int(cntInstId)]["tmpl"][tmpl_id]["equip"].update({
                            equip: {
                                "hide": 0,
                                "locked": 0,
                                "level": level
                    ...
                    myCharList[int(cntInstId)]["tmpl"][tmpl_id]["currentEquip"] = equip_table["charEquip"][tmpl_id][-1]
    ...
    if "nameCardV2Data" in display_meta_table:
        player_data["user"]["nameCardStyle"]["skin"]["state"] = {}
        for i in display_meta_table["nameCardV2Data"]["skinData"]:
            player_data["user"]["nameCardStyle"]["skin"]["state"][i] = {
                "unlock": True,
                "progress": None
            }
    ...
    # v2 ui
    charRotation = config["userConfig"]["charRotation"]
    ...
    if "user" in saved_data and config["userConfig"]["restorePreviousStates"]["ui"]:
        ...
        charRotation = saved_data["user"]["charRotation"]
    ...
    player_data["user"]["charRotation"] = charRotation
    ...
    if "trainingCampData" in story_review_meta_table:
        for i in story_review_meta_table["trainingCampData"]["stageData"]:
            player_data["user"]["trainingGround"]["stages"][i] = {
                "stageId": i,
                "state": 3,
                "rts": 1700000000
            }
